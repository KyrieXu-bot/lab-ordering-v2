...
export default function SalesDashboard() {
  ...
  async function load() {
    ...
    try { setRows((await getOrderRequests()).data) }
  ...
  async function withdraw(row) {
    ...
    try { await withdrawOrderRequest(row.request_id); await load() }
  ...
  return (
    <PortalLayout>
      ...
      <section className="portal-card">
        <div className="portal-card-heading"><div><h2>申请记录</h2><p>共 {rows.length} 条</p></div></div>
                  ...
                  <td className="portal-actions">
                    ...
                    {row.status === 'approved' && row.approved_order_id && (
                      row.pdf_generated && row.attachment_file_id
                        ? <button onClick={() => downloadPdf(row)}>下载PDF</button>
                        : generatingId === row.request_id
                          ? <span className="pdf-generation-progress"><span>生成中</span><i /></span>
                          : <button onClick={() => generatePdf(row)}>生成PDF</button>
                    )}
